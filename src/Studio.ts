...
export class StudioPanel extends FloatingPanel {
    ...
    private exportCanm() {
        ...
        for (let i = 0; i < trackNames.length; i++) {
            let track: CANM.Track;

            switch (i) {
                case 0: track = CANM.studio_track_to_track(this.animation.posXTrack.keyframes); break;
                case 1: track = CANM.studio_track_to_track(this.animation.posYTrack.keyframes); break;
                case 2: track = CANM.studio_track_to_track(this.animation.posZTrack.keyframes); break;
                case 3: track = CANM.studio_track_to_track(this.animation.lookAtXTrack.keyframes); break;
                case 4: track = CANM.studio_track_to_track(this.animation.lookAtYTrack.keyframes); break;
                case 5: track = CANM.studio_track_to_track(this.animation.lookAtZTrack.keyframes); break;
                case 6: track = CANM.studio_track_to_track(this.animation.bankTrack.keyframes); break;
                default: continue;
            }

            anim.tracks.get(trackNames[i])?.push(track);
            header.frame_count += track.values.length;
        }
        
        anim.header = header;
        
        const a = document.createElement('a');
        const data = new Blob([CANM.js_canm_to_bytes(anim)], { type: 'application/octet-stream' });
